"""BBC News - gender and women's rights coverage."""

from scraper.base import BaseScraper, HTMLScraper, ScrapedArticle, clean_html, RSSMixin


class BBCScraper(RSSMixin, BaseScraper):
    name = "BBC News"
    language = "EN"
    ...
    async def fetch_articles(self) -> list[ScrapedArticle]:
        ...
        for topic_url in self.TOPIC_URLS:
            try:
                html = await self._get(topic_url)
                from bs4 import BeautifulSoup
                soup = BeautifulSoup(html, "lxml")
                for link_tag in soup.select("a[href*='/news/articles/']"):
